(function newsApi() {
  let countries = [
    ...
    { state: "it", source: "la-repubblica" },

    { state: "de", source: "die-zeit" },

    { state: "fr", source: "liberation" },

    { state: "gb", source: "the-guardian-uk" }
  ];
  ...
  function getHeadlines(data) {
    ...
    while (br < 6) {
      if (
        ...
        $(`.headline${br + 1}`).innerHTML = `${
          data.articles[i].title
        } <a class="a a${br + 1}">   Read more</a>`;
        $(
          `.slider${br + 1}`
  ...
  function getFlashNews(data) {
    ...
    while (br < 6) {
      if (
        ...
        $(`.flash${br + 1}`).innerHTML = `<img class="img img${i + 1}" src="${
          data.articles[i].urlToImage
        }">${data.articles[i].title}`;
  ...
  function getTopNews(data) {
    for (let i = 0; i < data.articles.length; i++) {
      ...
      $(`.cell${i + 1}`).innerHTML = `<img class="img img${i + 1}" src="${
        data.articles[i].urlToImage
      }">`;
      $(`.cellp${i + 1}`).innerHTML = `<p><strong>Date: ${
        data.articles[i].publishedAt.split("T")[0]
      } <br><br>Title: ${data.articles[i].title}<br><br>Author: ${
  ...
  function getModalData(array, x) {
    ...
    if (array[x - 1].author !== null && array[x - 1].author !== "") {
      $(".modalAuthor").innerHTML = `Author: ${
        array[x - 1].author
      }<span class="modalDate">Date: ${array[x - 1].date}</span>`;
    } else {
      $(
        ".modalAuthor"
      ).innerHTML = `Author: Unknown<span class="modalDate">Date: ${
        array[x - 1].date
      }</span>`;
    }
